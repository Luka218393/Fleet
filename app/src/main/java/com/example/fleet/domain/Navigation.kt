...
object Navigation{
    fun goTo(screen: Screens, navigator: Navigator?, componentId: String = "1"){
        ...
        when(screen){
            ...
            Screens.DISPLAY_TENANT -> navigator?.push(DisplayScreen(tenantId = componentId))
            Screens.DISPLAY_APARTMENT -> navigator?.push(DisplayScreen(apartmentId = componentId))
            Screens.DISPLAY_BUILDING -> navigator?.push(DisplayScreen(buildingId = componentId))
